...
import { addedPilotIds, type AddPilotInput, type PilotPicture, type VideoWorkspaceConfig } from "@/lib/video-workspace";
...
export function AddPilotDialog({ workspace, disabled, onAdd, onClose }: {
  ...
  const [sourceId, setSourceId] = useState(() => countFor(workspace.activeSourceId) < 4 ? workspace.activeSourceId : "new");
  const [athleteCode, setAthleteCode] = useState("");
  const [picture, setPicture] = useState<PilotPicture>("full");
  const [error, setError] = useState<string | null>(null);
  const sourceFull = sourceId !== "new" && countFor(sourceId) >= 4;
  ...
  return <dialog className={styles.dialog} ref={dialogRef} aria-labelledby="add-pilot-title" onCancel={onClose}>
    <form onSubmit={(event) => {
      ...
      <label className={styles.field}>画面来源<select aria-label="画面来源" value={sourceId} disabled={disabled} onChange={(event) => setSourceId(event.target.value)}>
        {workspace.sources.map((source) => <option key={source.id} value={source.id} disabled={countFor(source.id) >= 4}>{source.label || "未命名输入"}{countFor(source.id) ? ` · 已关联 ${countFor(source.id)} 位飞手` : " · 尚未关联飞手"}</option>)}
