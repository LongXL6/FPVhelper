...
首次进入工作台先「添加飞手」：填写名称、选择已有或新增视频输入，再选择完整画面或四合一中的区域。每次确认只新增一个飞手窗口，可继续逐个添加；姓名和画面对应关系保存在本机。已保存的旧版布局继续保留，录制期间禁止添加或切换飞手。
...
与 FPVSuperApp 的所有权、身份、时间和本地数据迁移边界见 [模块契约（Phase 0 提案）](docs/designs/fpvhelper-module-contract.md)。契约不代表已完成平台接入。

- 通过浏览器 `getUserMedia` 打开 UVC HDMI 采集卡；可录制当前选手绑定的完整或裁切画面，将左右摇杆、刻度、选手代号和输入状态烧录进本地视频，并把原始打杆 JSON 保存到同一授权目录。浏览器支持时优先输出 MP4，不支持时在开始前明确显示 WebM；实际编码格式、文件后缀和保存收据保持一致。
- 每个画面上的“裁切与绑定”可直接打开该选手设置；一张四合一输入可分别选择四位选手的区域。
- 连接后自动读取当前 USB 飞控的 Pilot Name，空时回退 Aircraft/Craft Name；支持旧版机体名称查询。可直接修改为手动名字，再点击“使用飞控名称”恢复自动。已有手动名称继续保留，自动名称不跨设备或页面重载复用；录制时固定开始使用的名称。
- 展开「摇杆叠层」可分别调整左右摇杆尺寸（72–260 px，受画面边界约束）和不透明度；支持拖动定位、吸附后成组锁定、自动保存和重置，教练大屏中也可直接调整。小尺寸自动收起文字读数，保留打杆位置与轨迹。录像独立绘制叠层，按画面宽度比例读取当前尺寸、位置和不透明度，不依赖预览叠层是否显示；动态轨迹仅显示在预览中。
- 工作台内提供默认关闭的「ARM 自动记录 · Beta」：使用当前选手、画面、OSD 和保存目录；ARM 开关触发开始，连续 DISARM 15 秒后结束，期间重新 ARM 继续同一段。原有「结束记录」可手动保存；数据失联不视为 DISARM。设置及信号边界见 [ARM 自动记录 Beta](docs/arm-auto-record.md)。
- 通过 Web Serial 连接独立的地面桥接飞控，以只读 MSP v1 请求轮询 Betaflight：
  ...
  - 实时油门图以主机单调时间显示最近三秒，停收后旧点逐渐移出窗口；来源切换清空旧显示。分段检查抽点前相邻 RC 样本的 >50 ms、非正或无效间隔，沿用现有采集质量定义，不把显示抽点的正常间距当成 RF 丢包。显示缓冲最多 512 点，不改变原始记录。
  - `MSP_ANALOG`：地面桥接飞控电压与 legacy RSSI；该值不是机上 ELRS LQ。
  - 名称是首个 RC 帧后的一次性可选读取：`MSP_API_VERSION`、支持 API 1.45 及以上时的 MSPv2 `MSP2_GET_TEXT`，以及旧版 `MSP_NAME` 回退。名称查询不影响 RC 采样，失败时仍可手动命名。地面桥接飞控的配置名不一定等于 HDMI 中飞行器的名字，操作员需确认对应选手。
- 真实串口链路在线且自动读到或手动填写选手姓名/代号后，才能开始训练 Session；IndexedDB v2 分块追加保存，每秒尝试一次检查点，显示已经事务确认的帧数与时间。历史列表读取摘要，详情按需读取；导出继续使用兼容旧文件的 schema v2 JSON。
- 未连接硬件时可使用明确标记的演示数据；演示数据不计入试点生产记录。
- 不写入 Betaflight 配置，不上传视频，也不上传原始 RC 打杆样本；默认录制内容为“视频＋打杆 OSD＋数据”，仍需授权目录后手动开始或明确启用 ARM Beta，明确保存过的“仅数据”偏好继续保留。
...
| --- | --- | --- | --- |
| 视频帧、本地 MP4/WebM、DVR 原片 | 否 | 录制视频仅写入操作员授权的本地目录；俱乐部仍控制正式 DVR | FPVHelper 永不上传；选择“仅原始打杆数据”即可只看不录 |
...
授权目录中的 JSON 使用不覆盖的版本命名：首次为原 Session 文件名，再次导出遇到同名文件时保存为 `-v2.json`、`-v3.json`，保留所有旧文件。不同训练记录使用各自的 Session ID；同一记录的新版本保留原 ID 与视频关联。保存提示显示实际文件名，只有写入和关闭都成功才确认导出。已配置目录时，训练记录中的“导出 JSON”也沿用该目录；未配置时使用浏览器保存窗口或普通下载。文件名版本用于归档快照，不会复制视频。
...
选择“视频＋打杆 OSD＋数据”后，停止训练先结束采样，再等待 MP4/WebM 最后写入和 `close()`，之后将成功收据关联到 Session、完成本机保存并自动导出同目录 JSON。收据含实际文件名、MIME、字节数、录像起止时间与叠层说明。视频失败仍保存遥控数据，且不写入成功录像收据；JSON 与视频的保存结果分别显示。浏览器请求普通下载不等于确认文件落盘。MP4 探测支持后若编码器启动失败，会明确报告失败，不会通过改后缀或静默改格式伪装成功。
...
从“工作站设置 → 实验功能 → 录像视觉实验台”进入 `/vision-lab`，使用自己的门照片和录像，选择完整画面或四分之一画面，本地分析后逐个复核候选、计算起终点间隔，并保存门档案及 JSON/CSV。首次分析下载约 24.5 MB 模型与运行库，视频和参考图不上传；已接入真实参考匹配模型，当前仍需人工确认，尚不提供实时自动计圈或物理过门精度保证。详见[使用说明](docs/vision-lab.md)。

同一实验入口提供[实时过门实验](docs/live-vision.md)，首次打开才加载；离开该页停止识别并保留结果，视频和打杆录制继续。日常工作台默认收起遥控仪表、油门曲线与连接诊断，可展开“遥控详细数据”查看；原本机门档案和历史记录保留。

打杆实时监看也提供“实时过门计时”：共用门档案和当前选手的裁切画面，显示候选、已复核圈数和圈速，独立保存实时计时记录。需要同时录像时先开始训练记录，再开始过门计时。仍为单路实验功能，模型候选需人工确认；见[实时使用说明](docs/live-vision.md)。
