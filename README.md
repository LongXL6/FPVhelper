...
- 仅需 DVR 时，将“录制内容”切换为“仅视频 DVR（无需飞控）”，打开视频并选择目录即可录制；不要求姓名、飞控、遥控链路或训练数据库可用，不烧录打杆叠层，也不生成遥测 JSON。可复用已授权目录，另选的 DVR 目录仅保留在本页。
...
| 原始 RC 通道与高频样本 | 否 | 本机 IndexedDB；手动导出或按设置自动保存本地 JSON，使用视频＋打杆模式时一并保存到授权目录 | 永不自动上传 |
...
选择“视频＋打杆 OSD＋数据”后，停止训练先冻结并独立保存遥控数据；MP4/WebM 最后写入和 `close()` 完成后，再将成功收据关联到 Session 并自动导出同目录 JSON。收据含实际文件名、MIME、字节数、录像起止时间与叠层说明。视频失败仍保存遥控数据，且不写入成功录像收据；JSON 与视频的保存结果分别显示。浏览器请求普通下载不等于确认文件落盘。MP4 探测支持后若编码器启动失败，会明确报告失败，不会通过改后缀或静默改格式伪装成功。
