...
| 证据层 | 当前状态 | 可证明 | 不可证明 |
| --- | --- | --- | --- |
| 本地代码 | `3ea9b4e` 已完成 42 个 Vitest 文件 / 229 项、非增量 typecheck、lint、生产 build；Chromium 2 条流程各重复 3 次共 6/6；本地 Supabase pgTAP 2 文件 / 56 项通过 | 当前集成实现可编译，单元、假硬件浏览器主链、数据库策略与限流测试通过 | 真实 UVC、真实 ELRS、生产域、云数据库、真实 YOLO 模型 |
| GitHub PR | PR #1 已建立，`codex/fpvhelper-artifact-integration` 已推送到 `3ea9b4e` | 当前集成 SHA 可远端审查 | 已合并、生产发布 |
| GitHub Actions | runner 在执行任何 step 前因账户 Billing 被阻止；该 pre-runner 状态不阻塞试点 | workflow 已声明；失败不是代码或测试失败 | CI 测试结果；真正执行后的代码/测试失败仍会阻断 |
| Vercel Preview | `3ea9b4e` 对应 Preview 为 Ready | 当前 Preview 构建可访问 | 商用生产许可、生产 Promote、客户网络可用性 |
| Supabase 云端 | 已决定复用 FPVSuperApp 项目；真实 project ref 尚未从 FPVSuperApp 配置核验，当前可见 LONGWEBSITE 项目明确排除 | 本地 migration/pgTAP 可运行；共享项目归属已确定 | FPVSuperApp-owned production migration、专属命名空间、受限摄入、云端事件链 |
...
| 唯一客户域、内部验证域、FPVSuperApp 共享 Supabase | 外部待验证 | 域名与共享项目决策已确认 | 从 FPVSuperApp 真实配置核验 project ref；生产只统计客户域；不得使用 LONGWEBSITE 项目 |
...
| Phase 1 统计、Route、DB、周漏斗、验收记录 | 外部待验证 | 19 事件、白名单、工作站 token、替换流程、限流、migration 与本地 pgTAP 56 项已通过；内部域默认不发送 | FPVSuperApp-owned migration、受限摄入、书面确认、Vercel 环境和端到端事件链尚未建立 |
| 文案、离开本机的数据表、CI、Node 版本 | 集成分支已验证 | README、CLAUDE、CI、Node 24 已提交；本地门禁通过 | CI 若能运行则记录；账户/计费型 pre-runner 失败单列，不阻塞试点 |
| 版本提示、Preview、Promote、tag、CHANGELOG | 外部待验证 | `/version.json`、版本提示、release 文档、CHANGELOG 存在；当前全门禁和 Preview 通过 | 获得远端审查、批准 Vercel 商用方案后才可 Promote / tag；CI 作为补充证据 |
| Vercel/Supabase 商用、区域、配额、备份、限流 | 外部待验证 | Vercel 当前团队为 Hobby；Supabase 本地策略通过 | 有权负责人批准 Vercel 商用方案；核验共享项目、专属命名空间与受限摄入 |
...
- 浏览器假硬件 E2E；
- 独立代码审查；
- GitHub CI 如能运行则记录；账户/计费型 pre-runner 失败不阻塞，但真正执行后的代码/测试失败必须修复；
- Vercel Preview 对应 commit；
- FPVSuperApp 共享项目的 production migration、专属命名空间、受限摄入与事件链；
- 拆桨真机验收记录；
- 俱乐部书面确认与 DVR 基线。
