...
Sharing excludes raw host process-list bytes and removes unrelated process names/paths from JSON snapshots; original bytes stay local and original/shared hashes map each transformation. Full tracked A/B source tarballs are supplied and verified against Git blobs. Compiled build bytes, dependencies, Git object databases, the MP4 original bytes recorded in this formal experiment, and original Computer Use PNG bytes are not attached. The complete tracked-source tarballs do include the existing synthetic vision fixtures `e2e/fixtures/vision-synthetic-reference.png` and `e2e/fixtures/vision-synthetic-two-scenes.mp4`; these are not this experiment’s recorded media or real DVR footage. No source tarball is modified. The retained preflight truncated timeline is INVALID and remains bounded, not a parseable diagnostic. The valid preflight CPU profile remains Chrome-readable after sharing redaction.
