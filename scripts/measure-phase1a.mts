...
for (let i = 2; i < process.argv.length; i++) {
  ...
  if (!["--plan", "--normal-url", "--profile-url", "--output", "--condition", "--mode", "--run-index", "--exploratory"].includes(key) || args.has(key)) throw new Error(`Invalid argument ${key}`);
  if (key === "--exploratory") args.set(key, "true");
...
const sourceDiffSha256 = createHash("sha256").update(execFileSync("git", ["diff", "HEAD", "--"], { cwd: repo })).digest("hex");
...
async function interval(page: Page, label: string, durationMs: number, steady: boolean, withDrain: boolean): Promise<MeasurementWindow> {
  ...
  const measured = await page.evaluate(async ({ label, durationMs, steady, drainMs }) => {
    ...
    const t1 = stamp("end");
    // The source remains live during drain so normal recording does not become a stale-link experiment.
    // Reconciliation includes only independent IDs delivered in [t0,t1).
    const drainEnd = t1 + drainMs;
    if (drainMs) await new Promise((resolve) => window.setTimeout(resolve, drainMs));
    stamp("drain_observer_returned");
    return { label, t0, t1, drainEnd, steady };
  }, { label, durationMs, steady, drainMs: withDrain ? plan.drainMs : 0 });
  ...
  return { ...measured, cdp: { before, after, deltas, domain: "CDP renderer Performance counters; timestamps/duration deltas in seconds, heap in bytes. Observation brackets the page window plus drain; not whole-process CPU or physical input latency." } };
}
...
try {
  for (const planned of selected) {
    ...
    try {
      await Promise.race([
        ...
        (async () => {
          ...
          try { const cdp = await context.newCDPSession(page); await cdp.send("Performance.enable"); cdpSessions.set(page, cdp); }
    ...
    const result: MeasurementResult = { runId, condition, mode: planned.mode, repeat: planned.repeat, exploratory, windows, ...finalSnapshot, sessions, errors, networkViolations,
      networkCounts, fileEvidence, cleanup, sourceSha, sourceDiffSha256, planSha256, startedEpochMs, endedEpochMs: Date.now(), resourcesBefore,
